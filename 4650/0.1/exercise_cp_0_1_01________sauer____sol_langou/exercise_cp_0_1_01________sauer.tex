...
\hypersetup{
pdftitle={Langou :: Sauer CP.0.1.1 (question)},
...
\framebox{
\begin{minipage}{\textwidth}
\textbf{CP.0.1.1, Sauer}\\\\
Use {\texttt nest.m} to evaluate $p(x) = 1 + x + \ldots + x^{50}$ at $x=1.00001$.
(Use the Matlab {\texttt ones} command to save typing.)
Find the error of the computation by comparing with the equivalent expression 
$q(x) = (x^{51}-1)/(x-1)$.
\end{minipage}}
